...
class Knowledgebase(DataBaseModel):
    ...
    is_shared = IntegerField(default=0) # 标识是否共享知识库
...
# 用户数据库表     
class UserKnowledgebasePermission(DataBaseModel):
    id = CharField(max_length=32, primary_key=True)
    user_id = CharField(max_length=32, null=False, help_text="User ID")
    nickname = CharField(max_length=100, null=False, help_text="nicky name")
    email = CharField(max_length=255,null=False, help_text="email",index=True)
    
    knowledgebase_id = CharField(max_length=32, null=False, help_text="Knowledgebase ID")
    knowledgebase_name = CharField(max_length=128, help_text="Knowledgebase name", null=False)
    # 还有知识库的名称--这样就不用list知识库了
    permission = CharField(
        max_length=16,
        null=False,
        help_text="Permission for the knowledgebase",
        choices=["disabled", "view", "use"],
        default="view"
    )

    class Meta:
        db_table = "user_knowledgebase_permission"
        indexes = (
            (('user_id', 'knowledgebase_id'), True),  # 联合唯一索引，确保每个用户对每个知识库只有一条权限记录
        )

    def __str__(self):
        return f"User {self.user_id} - Knowledgebase {self.knowledgebase_id} Permission"
