...
def upload():
    ...
    for file in file_objs:
        try:
            ...
            print(f"File {file.filename} identified as type: {filetype}")
            if filetype == FileType.OTHER.value:
                raise RuntimeError("This type of file has not been supported yet!")

            location = filename
            while MINIO.obj_exist(kb_id, location):
                print(f"File {location} already exists, adding '_'")
                location += "_"
            print(f"Storing file at location: {location}")

            blob = file.read()
            print(f"Read file {file.filename} with size {len(blob)} bytes")
