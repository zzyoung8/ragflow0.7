...
def naive_merge(sections, chunk_token_num=128, delimiter="\n。；！？", chunk_overlap=0.5):
    ...
    for i in range(len(cks) - 1):
        overlap_length = int(len(cks[i]) * chunk_overlap)
        overlap_chunks.append(cks[i] + cks[i + 1][:overlap_length])
    overlap_chunks.append(cks[-1])
...
def naive_merge_docx(sections, chunk_token_num=128, delimiter="\n。；！？", chunk_overlap=0.5):
    ...
    for i in range(len(cks) - 1):
        overlap_length = int(len(cks[i]) * chunk_overlap)
        overlap_chunks.append(cks[i] + cks[i + 1][:overlap_length])
        overlap_images.append(images[i])
    overlap_chunks.append(cks[-1])
    overlap_images.append(images[-1])
    
    return overlap_chunks, overlap_images